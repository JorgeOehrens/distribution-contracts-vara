...
async fn do_something_works() {
    ...
    let program_code_id = remoting.system().submit_code(vft::WASM_BINARY);

    let program_factory = vft_client::VftFactory::new(remoting.clone());
    ...
    let mut service_client = vft_client::Vft::new(remoting.clone());
    ...
    assert_eq!(result, "Hello from Vft!".to_string());
}
...
async fn get_something_works() {
    ...
    let service_client = vft_client::Vft::new(remoting.clone());
