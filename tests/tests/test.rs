...
async fn test_basic_function() {
    ...
    let pool_code_id_ = program_space
        .system()
        .submit_code_file("./target/wasm32-unknown-unknown/release/pool.opt.wasm");

    println!("Contract Pool submitted with Code ID: {:?}", pool_code_id_);
