...
impl ExtendedService {
    pub fn seed(name: String, symbol: String, decimals: u8, admins: Vec<ActorId>) -> Self {
        unsafe {
            EXTENDED_STORAGE = Some(ExtendedStorage {
                admins: HashSet::from_iter(admins.clone()),
                minters: HashSet::from_iter(admins.clone()),
                burners: HashSet::from_iter(admins),
                shares_distributed: false, // Inicialización de la bandera
